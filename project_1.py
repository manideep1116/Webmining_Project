...
#def getRest(restaurant):
#    rest='NA'
#    NameChunk=restaurant.find('div',{'class':re.compile('media-block media-block--18')})
#    if NameChunk: rest=NameChunk.text
#    return rest

def getRestName(name):
    restname='NA'
    NameChunk=name.find('span')
    #NameChunk=name.find('h1',{'class':re.compile('biz-page-title embossed-text-white shortenough')})
    if NameChunk: restname=NameChunk.text
    return restname

def getRestType(name):
    cuisinetype='NA'
    NameChunk=name.find('a',{'href':re.compile('/search')})
    if NameChunk: cuisinetype=NameChunk.text
    return cuisinetype

def getReview(review):
    text='NA' # initialize text. NA because sometimes there are no reviews
    ReviewChunk=review.find('p',{'lang':re.compile('en')})
    if ReviewChunk: text=ReviewChunk.text#.encode('ascii','ignore'). text in that element is fetched, which is the name
    return text
...
def run(url):
    ...
    for p in range(1,pageNum+1): # for each page 
        ...
        names=soup.findAll('div',{'class':re.compile('biz-attributes')})
        restaurants=soup.findAll('div',{'class':re.compile('media-block media-block--18')})
        reviews=soup.findAll('div', {'class':re.compile('review review--with-sidebar')}) # get all the review divs
        
        for restaurant in restaurants:
            for name in names:
                restname=getRestName(name)
                cuisinetype=getRestType(name)
                print(restname,cuisinetype)
            
            for review in reviews:
                
                text=getReview(review)# finds and returns the name of the critic from the given review object
...
if __name__=='__main__':
    url='https://www.yelp.com/search?cflt=restaurants&find_loc=Hoboken%2C+NJ'
    run(url)
